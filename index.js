...
function getSort(a, b, num){
  if(roles[num] == '뜻'){
    if(a[num].split(/, |; /).some(el => el.split(' ¶')[0].match(/^${t}\s?\(/) || el.split(' ¶')[0].match(/\)\s?${t}$/) '+t) || el.split(' ¶')[0] == t)){
      return -1;
    }
    if(b[num].split(/, |; /).some(el => el.split(' ¶')[0].match(/^${t}\s?\(/) || el.split(' ¶')[0].match(/\)\s?${t}$/) '+t) || el.split(' ¶')[0] == t)){
      return 1;
    }
